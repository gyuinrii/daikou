  ...
  useEffect(() => {
    const initApp = async () => {
      ...
      async function loadData() {
        if (!db) return;
        try {
          var snapshot = await get(ref(db, DB_PATH));
          if (snapshot.exists()) {
      ...
      function saveData() {
        if (!db) return;
        set(ref(db, DB_PATH), { popups: D.popups, joints: D.joints, am: D.am })
      ...
      var changeHandler = function(e) {
        var t = e.target;
        var act = t.getAttribute('data-ch');
        if (act) handleChange(act, t);
      };
      var inputHandler = function(e) {
        var t = e.target;
        var act = t.getAttribute('data-inp');
        if (act) handleChange(act, t);
      };
      var keydownHandler = function(e) {
        if (e.key !== 'Enter') return;
        var t = e.target;
        var act = t.getAttribute('data-enter');
      ...
      function rHome(){
        ...
        if(D.joints.length){
          ...
          D.joints.forEach(function(j){
            var ns=0,paid=0,ship=0;
            (j.jproducts||[]).forEach(function(jp){ (jp.buyers||[]).forEach(function(b){ ns++; if(b.paid)paid++; if(b.shipped)ship++; }); });
      ...
      function pgProd(p){
        ...
        p.products.forEach(function(pr,i){
          h+='<div class="prod">'
            +'<div class="prod-top"><span class="prod-n">'+String(i+1).padStart(2,'0')+'</span>'
        ...
        h+='<button class="bg" style="width:100%;margin-top:4px" data-a="aPr" data-id="'+p.id+'">＋ 商品を追加</button>'
          +'<p class="hint">💡 KRW・JPYどちらか一方でも可。両方入力すると両方表示。</p></div>';
        return h;
      }
      ...
      function pgOrd(p){
        ...
        p.persons.forEach(function(pi,pIdx){
          ...
          h+='<div class="pc">'
            +'<div class="pc-hdr" data-a="togPc" data-id="'+p.id+'" data-pi="'+pIdx+'">'
            +'<div class="pc-av">'+person.charAt(0)+'</div>'
            +'<div style="flex:1;min-width:0"><div class="pc-name">'+esc(person)+'</div>'
            +'<div style="display:flex;gap:3px;margin-top:2px">'
            +(pi.paid?'<span class="bdg bg2">💰払済</span>':'')
            +(pi.delivered?'<span class="bdg bgold">📦渡済</span>':'')
            +(pi.delivery==='mail'?'<span class="bdg bp2">📮郵送</span>':'')
            +'</div></div>'
            ...
            +'<div class="pc-body'+(isOp?' op':'')+'">'; // body open
          h+='<div class="sts">'
          ...
          p.products.forEach(function(pr){
            ...
            h+='<div class="oitem">'
              +'<div class="oitem-top"><div class="oitem-info">'
              ...
              +'<div class="oitem-r">'
              +'<div class="osub'+(isZ?' z':'')+'" style="font-size:'+(ks&&js?'10px':'12px')+'">'+(isZ?'—':fB(ks,js))+'</div>'
            ...
            if(p.members.length){
              ...
              h+='<div class="mb-add" style="margin-bottom:6px">'
                ...
                +'</select>'
                +'<button class="mb-add-btn" data-a="addAM" data-id="'+p.id+'" data-pi="'+pIdx+'" data-rid="'+pr.id+'">追加</button>'
                +'</div>';
            }
            am.forEach(function(m){
              var q=(ord.mbQ[m])||0, mk=(Number(pr.price_krw)||0)*q, mj=(Number(pr.price_jpy)||0)*q;
              h+='<div class="mbr">'
                +'<div class="mbr-n">🎵 '+esc(m)+'</div>'
                +'<div class="qw">'
                +'<button class="qb" data-a="cMQ" data-id="'+p.id+'" data-pi="'+pIdx+'" data-rid="'+pr.id+'" data-m="'+esc(m)+'" data-delta="-1">−</button>'
            ...
            h+='<div class="mbr"><div class="mbr-n" style="color:var(--tx3);font-style:italic">指定なし</div>'
              +'<div class="qw">'
              +'<button class="qb" data-a="cNQ" data-id="'+p.id+'" data-pi="'+pIdx+'" data-rid="'+pr.id+'" data-delta="-1">−</button>'
      ...
      function rJoint(){
        ...
        (j.jproducts||[]).forEach(function(jp,pi){
          ...
          h+='<div class="jprod">'
            +'<div class="jprod-hdr" data-a="togJp" data-id="'+j.id+'" data-pi="'+pi+'">'
            +'<div class="jprod-ico">🛒</div>'
            +'<div style="flex:1;min-width:0">'
            +'<div class="jprod-name">'+(jp.name||'（商品名未設定）')+'</div>'
            +'<div class="jprod-meta">'+jp.buyers.length+'枠　💰'+bpaid+'/'+jp.buyers.length+'　📦'+bship+'/'+jp.buyers.length+'　合計'+fJ(bTotal)+'</div>'
            +'</div>'
            +'<div class="jprod-chev'+(isOp?' op':'')+'">▼</div></div>'
            +'<div class="jprod-body'+(isOp?' op':'')+'">'; // body
          ...
          jp.buyers.forEach(function(b,bi){
            h+='<div class="buyer">'
              +'<div class="buyer-top">'
              +'<input class="buyer-name-inp" value="'+esc(b.person||'')+'" placeholder="購入者名（例: Aちゃん）" data-ch="updBuyer" data-id="'+j.id+'" data-pi="'+pi+'" data-bi="'+bi+'" data-f="person"/>'
              +'<button class="bdel" data-a="dBuyer" data-id="'+j.id+'" data-pi="'+pi+'" data-bi="'+bi+'">🗑</button></div>'
              +'<input class="buyer-mb-inp" value="'+esc(b.member||'')+'" placeholder="メンバー（例: ジウン）" data-ch="updBuyer" data-id="'+j.id+'" data-pi="'+pi+'" data-bi="'+bi+'" data-f="member"/>'
              +'<div class="price-row">'
              +'<div class="pf-l">¥</div>'
              +'<input class="price-jpy-inp" type="number" value="'+esc(b.price||'')+'" placeholder="金額（円）" data-ch="updBuyer" data-id="'+j.id+'" data-pi="'+pi+'" data-bi="'+bi+'" data-f="price"/>'
              +'</div>'
        ...
        if(D.joints.length){
          ...
          D.joints.forEach(function(j){
            ...
            h+='<div class="card2"><div style="font-weight:700;margin-bottom:6px">'+esc(j.name)+'</div>'
              ...
              +'<span style="color:var(--tx3)">'+(j.jproducts||[]).length+'商品　'+ns+'枠</span>'
              +'</div>';
            (j.jproducts||[]).forEach(function(jp){
              ...
              h+='<div style="text-align:right;font-size:11px;color:var(--tx3);margin-top:5px">小計 <span class="mono" style="color:var(--ac)">'+fJ(bT)+'</span>　💰'+bP+'/'+jp.buyers.length+'　📦'+bS+'/'+jp.buyers.length+'</div>'
                +'</div>';
            });
            h+='</div>';
          });
    ...
    let cleanup;
    initApp().then(function(fn){ cleanup = fn; });
